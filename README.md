...
TiebaPure-iOS 是一个使用 SwiftUI 编写、以浏览为主的非官方百度贴吧客户端，支持访客浏览、首页推荐、进吧、吧内列表、搜索主题/回复、帖子与楼中楼、图片和视频查看，以及可选的百度手机号验证码登录。登录后还可关注或取消关注用户，并对主贴、楼层和楼中楼点赞或取消点赞；当前不提供发帖或回复入口。它与百度公司、百度贴吧官方及 TiebaLite 原作者均无隶属、授权或认可关系。
...
- 卸载后重装即视为全新安装：首次启动会清除卸载前遗留在 Keychain 中的登录凭证。
- 登录页面和网络请求限制为安全的 HTTPS 地址，并对 Cookie、缓存和响应大小进行约束。
- 帖子浏览历史最多保存 500 条，仅保存在本机数据库（SwiftData），可在“我的 → 浏览历史”逐条删除或全部清空；旧版 UserDefaults 数据首次启动时自动迁移。
- 支持本机关键词/用户/吧屏蔽（“设置 → 屏蔽设置”），规则只存本机；登录后可在“我的 → 消息”查看回复我的与 @ 我的。
- 支持 `tiebapure://thread/...`、`tiebapure://forum/...`，也可用 `tiebapure://open?url=<经百分号编码的 HTTPS 贴吧链接>` 包装后打开帖子或吧。普通 `https://tieba.baidu.com/...` 链接本身不是本项目的 Universal Link，不能直接保证唤起应用。
- 帖子收藏和阅读位置各最多保存 500 条，同样只保存在本机；可在“我的 → 帖子收藏”分别管理，互不影响。再次打开帖子时自动恢复到上次读到的楼层（按正序定位），横幅可一键回到顶部；滚回顶部即清除该帖的阅读位置。
